...
class WebGLPointsLayerRenderer extends WebGLLayerRenderer {
  ...
  constructor(layer, options) {
    ...
    this.listenKey_ = listen(source, VectorEventType.ADDFEATURE, function(evt) {
      ...
      this.propsMap[uid] = feature.getProperties();
      this.geometryMap[uid] = feature.getGeometry();
    }, this);
  }
  ...
  rebuildBuffers_(frameState) {
    ...
    const featureUids = Object.keys(this.geometryMap);
    ...
    let featureUid, geometry, feature, properties;
    const tmpCoords = [];
    const tmpColor = [];
    let renderIndex = 0;
    let hitIndex = 0;
    ...
    for (let i = 0; i < featureUids.length; i++) {
      ...
      geometry = this.geometryMap[featureUid];
      feature = this.featureMap[featureUid];
      properties = this.propsMap[featureUid];
    ...
    console.log('Time spent in rebuildBuffers: ', Date.now() - start);
  }
